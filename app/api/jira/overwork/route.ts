...
export async function GET() {
  try {
    ...
    // JQL: Find issues from Product: inCommerce project created in the last 6 months with workratio > 100, excluding Done
    // Using exact project name from Jira and workratio filter directly in JQL
    const jql = `project = "Product: inCommerce" AND created >= -24w AND workratio > 100 AND status != Done ORDER BY updated DESC`;
